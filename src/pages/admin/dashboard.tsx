...
const AdminDashboard = () => {
  ...
  const [section, setSection] = useState<'orders' | 'wallets' | 'changePassword'>('orders'); // Add 'changePassword' to the section state
        ...
        <div className="relative px-4 py-10 bg-white shadow-lg sm:rounded-3xl sm:p-20">
          ...
          <div className="flex justify-center mb-6">
            ...
            <button
              ...
              className={`px-4 py-2 mr-2 font-bold rounded ${section === 'wallets' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
            >
              Wallets
            </button>
            <button
              onClick={() => setSection('changePassword')} // Add button for Change Password section
              className={`px-4 py-2 ml-2 font-bold rounded ${section === 'changePassword' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
            >
              Change Password
            </button>
          </div>
          ...
          {section === 'changePassword' && <ChangePassword />} {/* Render ChangePassword component */}
        </div>
      </div>
    </div>
  );
